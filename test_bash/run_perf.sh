# Bash script for running experiments with ideal parameters with PerfEvent
...
for index in "${!file_array[@]}"; do
    ...
    g++ test_perf.cpp -march=native -fopenmp -std=c++17 -O3 -w -o z_run_perf && ./z_run_perf ${file_array[$index]} >> log_flirt_perf.txt
done
...
for index in "${!file_array[@]}"; do
    ...
    g++ test_perf.cpp -march=native -fopenmp -std=c++17 -O3 -w -o z_run_perf && ./z_run_perf ${file_array[$index]} >> log_bp_pgm_perf.txt
done
...
for index in "${!file_array[@]}"; do
    ...
    echo "#define WINDOWSIZE 100000000" >> parameters_perf.h;
    echo "#define MAXSEG 1" >> parameters_perf.h;
    g++ test_perf.cpp -march=native -fopenmp -std=c++17 -O3 -w -o z_run_perf && ./z_run_perf ${file_array[$index]} >> log_queue_perf.txt
done


for index in "${!file_array[@]}"; do
    echo "ALEX , ${file_array[$index]}";
    > parameters_perf.h;
    echo "#pragma once" >> parameters_perf.h;
    echo "#define ERROR 4" >> parameters_perf.h;
    echo "#define NO_EXECUTION 10000" >> parameters_perf.h;
    ...
    echo "#define WINDOWSIZE 100000000" >> parameters_perf.h;
    echo "#define MAXSEG 1" >> parameters_perf.h;
    g++ test_perf.cpp -march=native -fopenmp -std=c++17 -O3 -w -o z_run_perf && ./z_run_perf ${file_array[$index]} >> log_alex_perf.txt
done
