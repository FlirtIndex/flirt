# Bash script for running experiments with varying read-to-write ratios with PerfEvent
...
for index in "${!file_array[@]}"; do
    for num in "${!update_array[@]}"; do
        ...
        echo "#define NUM_UPDATE ${update_array[$num]}" >> parameters_perf.h;
        echo "#define NUM_SEARCH ${search_array[$num]}" >> parameters_perf.h;
        echo "#define NO_EXECUTION ${execution_array[$num]}" >> parameters_perf.h;
        ...
        g++ test_perf_rw.cpp -march=native -fopenmp -std=c++17 -O3 -w -o z_run_ratio && ./z_run_ratio ${file_array[$index]} >> log_flirt_perf_rw_${rw_ratio_array[$num]}.txt
done
done
...
for index in "${!file_array[@]}"; do
    for num in "${!update_array[@]}"; do
        ...
        g++ test_perf_rw.cpp -march=native -fopenmp -std=c++17 -O3 -w -o z_run_ratio && ./z_run_ratio ${file_array[$index]} >> log_bp_pgm_perf_rw_${rw_ratio_array[$num]}.txt
done
done
...
for index in "${!file_array[@]}"; do
    for num in "${!update_array[@]}"; do
        ...
        echo "#define WINDOWSIZE 100000000" >> parameters_perf.h;
        echo "#define MAXSEG 1" >> parameters_perf.h;
        g++ test_perf_rw.cpp -march=native -fopenmp -std=c++17 -O3 -w -o z_run_ratio && ./z_run_ratio ${file_array[$index]} >> log_queue_perf_rw_${rw_ratio_array[$num]}.txt
done
done
...
for index in "${!file_array[@]}"; do
    for num in "${!update_array[@]}"; do
        ...
        echo "#define WINDOWSIZE 100000000" >> parameters_perf.h;
        echo "#define MAXSEG 1" >> parameters_perf.h;
        g++ test_perf_rw.cpp -march=native -fopenmp -std=c++17 -O3 -w -o z_run_ratio && ./z_run_ratio ${file_array[$index]} >> log_alex_perf_rw_${rw_ratio_array[$num]}.txt
done
done
