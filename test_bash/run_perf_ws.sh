# Bash script for running experiments with varying window size with PerfEvent
...
do
    ...
    g++ test_perf.cpp -march=native -fopenmp -std=c++17 -O3 -w -o z_run_perf && ./z_run_perf $filename >> log_flirt_queue_perf_ws.txt
...
for index in "${!file_array[@]}"; do
    for windowsize in 10000 1000000 100000000; do
        ...
        g++ test_perf.cpp -march=native -fopenmp -std=c++17 -O3 -w -o z_run_perf && ./z_run_perf ${file_array[$index]} >> log_bp_pgm_perf_ws.txt
done
done
...
for index in "${!file_array[@]}"; do
    for windowsize in 10000 1000000 100000000; do
        ...
        echo "#define WINDOWSIZE $windowsize" >> parameters_perf.h;
        echo "#define MAXSEG 1" >> parameters_perf.h;
        g++ test_perf.cpp -march=native -fopenmp -std=c++17 -O3 -w -o z_run_perf && ./z_run_perf ${file_array[$index]} >> log_queue_perf_ws.txt
done
done
...
for index in "${!file_array[@]}"; do
    for windowsize in 10000 1000000 100000000; do
        ...
        echo "#define WINDOWSIZE $windowsize" >> parameters_perf.h;
        echo "#define MAXSEG 1" >> parameters_perf.h;
        g++ test_perf.cpp -march=native -fopenmp -std=c++17 -O3 -w -o z_run_perf && ./z_run_perf ${file_array[$index]} >> log_alex_perf_ws.txt
done
done
